...
| `weekly_events_announcement_job` | Default `false`. StonesBot's Sunday 12PM announcement of the coming week's calendar events (Monday–Sunday) in the church announcements channel. Each run posts a new embed and deletes the previous week's message, which is tracked in the `weekly_events_announcements` table. |
| `friday_pickups_summary_job` | Post the `/list-pickups-friday` embed to the ride coordinators channel, with a link to the dashboard's Ask Rides Overview (Friday tab). Default schedule is Friday 11AM. A per-slot on/off toggle and custom day/time live in Site Settings (`pickup_summary_settings` table); this flag gates both. Skipped during Wednesday-fellowship season, when the Friday ask-rides job is paused, or when no Friday ask-rides message went out that week. |
| `sunday_pickups_summary_job` | Same as above for `/list-pickups-sunday` (Sunday tab). Default schedule is Saturday 4PM. Skipped when the Sunday ask-rides job is paused or no Sunday message went out that week. |
| `temp_driver_expiry_job` | Seeded **on** by migration `c09138b28076` (grants aren't gated by it, so it must not start off). Sweep the `temp_driver_grants` table every 5 minutes (and once at startup) and remove the Driver role from anyone whose temporary grant has expired, posting a notice in the ride coordinators channel. **While off, temporary drivers keep the role past their expiry** until it's turned back on; the next sweep then catches up. Auto-disabled locally; use `/test-expire-temp-drivers` instead. |
