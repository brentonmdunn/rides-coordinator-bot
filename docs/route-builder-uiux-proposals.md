...
Inspection only — no code changes made. Files reviewed:
...
I've also noticed `frontend/src/components/RouteBuilder.tsx` (the pre-refactor flat file) is still present; it's not imported by `Home.tsx`, only the modular folder version is. Worth deleting separately, but mentioning here because that file currently houses a near-duplicate of all the issues below — when you fix one, fix the other or remove the dead one first.

Suggestions are grouped by priority. P1 = high impact / low cost, P2 = medium, P3 = nice-to-have.

---

## 1. Adding stops — flow inconsistencies (P1)
...
## 2. Mini-map vs fullscreen visual parity (P1)
...
## 4. Route output — surface useful trip metadata (P1)
...
## 6. Driver selector placement (P2)
...
## Quick-win shortlist (if you only do five)
