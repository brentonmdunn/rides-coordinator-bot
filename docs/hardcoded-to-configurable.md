...
- `WILDCARD_DATES = ["6/20/25", "6/27/25", "6/29/25"]` — `backend/ridebot/jobs/ask_rides.py:59`
...
- Message text: "leave between 7 and 7:10pm" / "10 and 10:10am" / "8:30 and 8:40am"
  — `backend/ridebot/jobs/ask_rides.py:117-153`
- Duplicated as real logic in `backend/ridebot/services/group_rides_service.py:39-40`
...
- `BOT_REACTIONS` — `backend/ridebot/jobs/ask_rides.py:186-191`
  (🪨 Wed/Fri, 🍔 🏠 ✳️ Sunday service, 📖 Sunday class)
- Driver emoji sets — `backend/ridebot/services/driver_service.py:24-41`
- Emoji constants — `backend/shared/core/enums.py:215-240`
...
- Monday 11 AM Wednesday-fellowship reminder + Wednesday-noon main job —
  `backend/ridebot/cogs/job_scheduler.py:34-80`, `backend/ridebot/utils/constants.py:117-118`
...
- `PickupLocations` / `CampusLivingLocations` enums — `backend/shared/core/enums.py:117-152`
- GPS coordinates / Maps links — `backend/ridebot/utils/constants.py:21-34`
- Location group lists (scholars / warren+pepper canyon / rita) — `backend/ridebot/utils/constants.py:54-72`
- Campus → pickup mapping — `backend/ridebot/services/group_rides_service.py:43-55`
...
- Channel IDs (rides announcements, driver chat, logs, driver spam, new-rides
  category) — `backend/shared/core/enums.py:6-21, 114`
- Role IDs (driver, rides, ride coordinator) — `backend/shared/core/enums.py:51-53`
- Command channel whitelists — `backend/ridebot/utils/channel_whitelist.py:11-20`
- Ride-coverage scanned channels — `backend/ridebot/services/ride_coverage_service.py:136-139`
- Agent allowed channels — `backend/ridebot/cogs/agent.py:24-30`
- Welcome-message channel — `backend/ridebot/services/ride_request_service.py:83`
...
- New-rider welcome message — `backend/ridebot/services/ride_request_service.py:82-87`
- Ask-rides header text ("for this week!" / "for Sunday Service!") — `backend/ridebot/jobs/ask_rides.py:424-426`
- Embed titles + colors per ride type (`RIDE_TYPES_CONFIG`) — `backend/ridebot/jobs/ask_rides.py:161-178`
...
- **Cache TTLs / thresholds** — `backend/ridebot/utils/constants.py:88-111` (session TTL,
...
- **Active-hours / ride-cycle time windows** — `backend/ridebot/utils/constants.py:115-117`,
  `backend/ridebot/utils/time_helpers.py:52-135`. Deeply woven into caching and coverage
