...
class RideCoverage(commands.Cog):
    ...
    async def sync_ride_coverage(self) -> dict:
        ...
        for channel_id in channels_to_scan:
            ...
            logger.info(f"sync_ride_coverage: Scanning channel {channel.name} (id={channel_id}) since {since}")
            ...
            try:
                async for message in channel.history(after=since, limit=500):
                    ...
                    if passenger_usernames:
                        logger.debug(f"sync_ride_coverage: Found grouping message {message.id} with {len(passenger_usernames)} passengers")
                        try:
        ...
        try:
            ...
            for orphaned_id in orphaned_ids:
                logger.info(f"sync_ride_coverage: Removing entries for deleted message {orphaned_id}")
                deleted = await self.repo.delete_all_entries_by_message(orphaned_id)
                total_entries_removed += deleted
                
            if orphaned_ids:
                logger.info(f"sync_ride_coverage: Cleaned up {total_entries_removed} orphaned entries from {len(orphaned_ids)} deleted messages")
        except Exception as e:
            logger.error(f"sync_ride_coverage: Error during orphan cleanup: {e}")
        
        logger.info(f"sync_ride_coverage: Completed. Scanned {total_messages_scanned} messages, added {total_entries_added} entries, removed {total_entries_removed} orphaned entries")
    ...
    def _extract_passengers(self, message: discord.Message) -> list[str]:
        ...
        for user in message.mentions:
            ...
            if any(
                m in content[drive_idx:]
                for m in (f"<@{user.id}>", f"<@!{user.id}>")
            ):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if passenger_usernames:
            logger.info(f"on_message: Detected ride coverage for: {passenger_usernames} (message_id={message.id})")
            try:
                await self.repo.add_coverage_entries(passenger_usernames, str(message.id))
                logger.info(f"on_message: Successfully added {len(passenger_usernames)} coverage entries")
            except Exception as e:
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message):
        ...
        if added:
            logger.info(f"on_message_edit: Ride coverage added via edit: {list(added)} (message_id={after.id})")
            try:
        ...
        if removed:
            logger.info(f"on_message_edit: Ride coverage removed via edit: {list(removed)} (message_id={after.id})")
            try:
                await self.repo.delete_coverage_entries(list(removed), str(after.id))
                logger.info(f"on_message_edit: Successfully removed {len(removed)} coverage entries")
            except Exception as e:
    ...
    async def on_message_delete(self, message: discord.Message):
        ...
        try:
            ...
            if deleted_count > 0:
                logger.info(f"on_message_delete: Removed {deleted_count} coverage entries for deleted message {message.id}")
            else:
                logger.debug(f"on_message_delete: No coverage entries found for message {message.id}")
        except Exception as e:
