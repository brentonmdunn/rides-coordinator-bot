...
async def get_pickups_by_message(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Message ID and Channel ID must be valid integers"
        )
    ...
    try:
        ...
        logger.info(f"Got {len(locations_people)} locations, {len(usernames_reacted)} users reacted")
                        ...
                        for person_tuple in locations_people.get(location, []):
                            if person_tuple[0] == name:
                                people_with_usernames.append({
                                    "name": person_tuple[0],
                                    "discord_username": str(person_tuple[1]) if person_tuple[1] else None
                                })
                                break
                    
                    result["housing_groups"][group_name]["locations"][location] = people_with_usernames
        
        logger.info(f"✅ Returning grouped result with {len(result['housing_groups'])} housing groups")
        return result
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch pickups: {str(e)}"
        )
